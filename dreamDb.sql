...
begin
    return query (
        select format('%s[%s]', course_name, sec_name) as name,
               full_name(first_name, last_name)        as instructor,
               instructor_id,
               class_begin,
               class_end,
               loc,
        ...
        from (
                 select course_name,
                        ...
                        sem_begin
                 from learning_info
                          join student_info si on learning_info.sid = si.sid
                          join section s on s.sec_id = learning_info.sec_id
        ...
        where substr(a.week_list,
                     cast(ceil((date_part('day', date(date1)) - date_part('day', a.sem_begin)) / 7) as int), 1) = '1'
    );
end;
$$;
